...
class LikedPost extends Component {
  ...
  renderProduct = () => {
    ...
    <View style={{
      width: width,
      height: this.hasComments(comments) ? height * 1.1 : height,
      backgroundColor: '#fff',
      alignSelf: 'center',
      borderWidth: 1,
      borderRadius: 2,
      ...
      <View style={{height: 400.2, backgroundColor: 'lightblue'}}>
        <ScrollView
          horizontal
          pagingEnabled
        ...
        >
          {this.props.data.images.map((image, index) =>
            <Image key={index} style={{width: width, height: '100%', backgroundColor: 'grey'}} source={{uri: image.url}}/>
          )}
        </ScrollView>
      </View>
      ...
      <View style={{width: '100%', height: '40%', marginTop: 16}}>
...

        {
          ...
          ?
          <View style={{ width, height: '40%', padding: 16 }}>
  ...
  renderLook = () => {
    let { comments } = this.props.data
    return(
      <View style={{width: width, height: this.hasComments(comments) ? height * 1.1 : height, backgroundColor: '#fff', marginBottom: 4, alignSelf: 'center', borderWidth: 1,
    borderRadius: 2,
    borderColor: '#ddd',
    borderBottomWidth: 0,
    shadowColor: '#000',
...
export default connect(mapStateToProps, actions)(LikedPost)
